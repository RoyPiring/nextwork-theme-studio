 ...
 * Paved paths join everything inside the fence; dirt roads run from the
 * gates to the learners' plots. Staff and visitors walk the paths, and
 * visitors arrive by car and bike. Nothing stands on a road. */
'use strict';
(function () {
  ...
  const LAKE = at(20, -25).concat([9, 7]); const JETTY = at(24, -17);
  const PADDOCK = at(-29, -26).concat([8, 5]); const BARN = at(-20, -26); const PADDOCK_GATE = at(-21, -20);
  ...
  line(at(EAST_AVE, -22), at(EAST_AVE, -20)); line(at(EAST_AVE, -20), JETTY); line(at(WEST_AVE, -22), PADDOCK_GATE); line(at(0, -9), NORTH_GATE);
  ...
  function roads(os) {
    ...
    const set = new Set(), tiles = [at(0, 34), at(-1, 34), at(1, 34), NORTH_GATE.slice()]; const l = (x, y) => set.add(key(x, y));
  ...
  const walkableAll = set => { const u = new Set(PAVED); ASPHALT.forEach(k => u.add(k)); set.forEach(k => u.add(k)); return u; };

  /* ---- the staff and visitors: somewhere to be, a while there, on again ---- */
  const POIS = { hall: HALL_DOOR, cafe: CAFE_DOOR, jetty: JETTY, paddock: PADDOCK_GATE, gate: SOUTH_GATE, hub: i => HUB_DOOR[i], bench: i => { const b = BENCHES[i]; let best = null, bd = 1e9; PAVED.forEach(k => { const t = k.split(',').map(Number); const d = Math.abs(t[0] + 0.5 - b[0] - 0.5) + Math.abs(t[1] + 0.5 - b[1] - 0.5); if (d < bd) { bd = d; best = t; } }); return best; }, space: i => SPACES[i].map(Math.round), rack: () => RACK_IN };
  const STAFF = [
    ...
    { name: 'Guide', plan: ['gate', 'hub0', 'hub1', 'hub2', 'hub3', 'hub4', 'hub5', 'hub6', 'hub7'], says: 'Every hub is a roadmap. Pick the one you keep thinking about.' },
  ...
  const VISITORS = [{ colour: '#3fa66b', car: '#3b7dd8', space: 1, plan: ['space1', 'gate', 'hub2', 'cafe', 'bench13', 'space1'] }, { colour: '#8f5fd1', car: '#e8552f', space: 5, plan: ['space5', 'gate', 'hub5', 'hall', 'space5'] }, { colour: '#f2b42a', bike: true, plan: ['rack', 'gate', 'hub6', 'bench14', 'rack'] }];
  const visitors = VISITORS.map((v, i) => ({ st: { name: 'Visitor', plan: v.plan }, i: 20 + i, at: poi(v.plan[0]).map(x => x + 0.5), step: 0, path: null, dwell: 6 + i * 7, colour: v.colour, car: v.car, space: v.space, bike: v.bike, away: false }));
  const CAR_COLOURS = ['#3b7dd8', '#e8552f', '#f2b42a', '#3fa66b', '#9aa3b0', '#8f5fd1'];
  let lastNow = 0;
  function tick(now, os) {
  ...
  function drawHQ(I, state, now, opts) {
    const ctx = I.ctx, os = owners(state), rd = roads(os); opts = opts || {}; tick(now, os);
    for (let gy = 0; gy < MAP; gy++) for (let gx = 0; gx < MAP; gx++) { if (!I.onScreen(gx, gy)) continue; if (asphalt(gx, gy)) { I.tile(gx, gy, '#6f737b', 'rgba(0,0,0,.15)'); continue; } if (paved(gx, gy)) { I.tile(gx, gy, (gx + gy) % 2 ? '#e3dccb' : '#d8d0bc', 'rgba(0,0,0,.06)'); continue; } if (rd.has(key(gx, gy))) { I.tile(gx, gy, '#c8a877', 'rgba(110,75,30,.35)'); continue; } if (lawn(gx, gy)) { I.tile(gx, gy, (gx + gy) % 2 ? '#7fc55a' : '#86cc60'); continue; } I.tile(gx, gy, ground(gx, gy)); }
    ...
    if (!opts.map) for (let gy = 0; gy < MAP; gy += 1) for (let gx = 0; gx < MAP; gx += 1) { const r = S.hash(gx * 7 + 1, gy * 3 + 2); if (r > 0.03 || !I.onScreen(gx, gy) || paved(gx, gy) || asphalt(gx, gy) || rd.has(key(gx, gy)) || inside(gx, gy) || (Math.abs(gx - C[0]) < 16 && gy > C[1] + 22)) continue; if (os.some(o => Math.abs(o.at[0] - gx) < 3 && Math.abs(o.at[1] - gy) < 3)) continue; add(gx + gy + 0.5, () => B.oak(I, gx, gy, 0.8 + S.hash(gy, gx) * 0.5)); }
    ...
    late(() => { I.label(HALL[0] + 3, HALL[1] + 4.6, 'NextWork Headquarters', 'Austin, Texas', 12); I.label(CAFE[0] + 1.5, CAFE[1] + 2.5, 'NextWork Cafe', 'ask anything', 9); I.label(LAKE[0] + 4.5, LAKE[1] + 8.2, 'The lake', '', 8); I.label(PADDOCK[0] + 4, PADDOCK[1] + 6, 'The paddock', '', 8); I.label(C[0] + 8, C[1] + 32.6, 'Parking', '', 8); I.label(RACK[0] + 1.5, RACK[1] + 1.9, 'Bike racks', '', 8); I.label(C[0] + 0.5, C[1] + 45, 'The entry road', 'from town', 8); });
  ...
  NW.HQ = { MAP, C, PLOTS, MINE, owners, drawHQ, hitHQ, HUB_AT, HALL, CAFE, FENCE, ARCH, LOT };
})();
