...
(function () {
  ...
  const CSS = `
    ...
    .badge { position: absolute; left: 8px; top: 8px; font: 800 12px/1.2 "Baloo 2", system-ui, sans-serif; color: #172033; background: rgba(255,255,255,.88); padding: 6px 10px; border-radius: 999px; border: 1px solid rgba(0,0,0,.12); } .badge small { font-weight: 700; color: #5b6478; margin-left: 6px; }
    .nav { position: absolute; right: 8px; bottom: 8px; display: flex; gap: 5px; flex-wrap: wrap; justify-content: flex-end; max-width: 75%; }
    .nav button, .devbar button { font: 700 11px/1 "Baloo 2", system-ui, sans-serif; padding: 6px 9px; border: 1px solid rgba(0,0,0,.12); border-radius: 999px; background: rgba(255,255,255,.88); color: #172033; } .nav button:hover, .devbar button:hover { background: #fff; }
    ...
    .who { background: rgba(255,255,255,.88); border: 1px solid rgba(0,0,0,.12); border-radius: 10px; padding: 4px 9px 5px; min-width: 120px; } .who b { font: 800 12px/1.1 "Baloo 2", system-ui, sans-serif; color: #172033; display: block; } .who small { display: block; font-size: 10px; color: #5b6478; font-weight: 700; white-space: nowrap; }
    ...
    .devtoggle { font: 700 11px/1 "Baloo 2", system-ui, sans-serif; margin: 6px 12px 0; padding: 5px 10px; border: 1px solid rgba(143,227,164,.4); border-radius: 999px; background: transparent; color: #8fe3a4; align-self: flex-start; } .devtoggle[hidden] { display: none; }
    ...
    .zoom { position: absolute; left: 8px; bottom: 8px; display: flex; gap: 4px; } .zoom button { width: 30px; height: 30px; font: 800 16px/1 "Baloo 2", system-ui, sans-serif; border: 1px solid rgba(0,0,0,.12); border-radius: 8px; background: rgba(255,255,255,.88); color: #172033; }
  ...
  function mount(root, host) {
    ...
    let current = 'world';
    function show(id) { current = id; TABS.forEach(([k]) => { tabBtns[k].setAttribute('aria-selected', String(k === id)); views[k].hidden = k !== id; }); if (id === 'build') paintBuild(); if (id === 'base') paintBoard(); host.remember && host.remember('tab', id); if (typeof fit === 'function') setTimeout(fit, 0); }
    ...
    nav(b1.sc, [['Home', () => { Land.goTo(me, lastL.paths, [S.HOME[0] + 1.5, S.HOME[1] + 2.5]); me.follow = true; }], ['World', () => show('world')], ['Build', () => show('build')]]);
    ...
    function paintDash() { const era = E.eraOf(state), ne = E.nextEra(state), nx = S.nextProject(state); dashT.textContent = S.landName(state) + ' · ' + era.word + ' · ' + E.score(state) + ' built'; while (dashList.firstChild) dashList.removeChild(dashList.firstChild);
      ...
      step(2, 'This is ' + era.word, 'It came with ' + era.needs + '.'); step(3, ne ? (ne.n - E.score(state)) + ' more and it is ' + ne.word : 'A kingdom. The whole thing.', ne ? 'That brings ' + ne.needs + '.' : 'Every era, earned.'); step(4, 'Walk the roads', 'Tap anywhere and you take the roads there. Arrow keys walk too.'); if (state.lists.length) step(5, state.lists.length + ' learn lists over the ' + (E.level(state) >= 6 ? 'river' : 'creek'), 'Your own projects, in rows of their own.'); }
    ...
    drag(b1.cv, base, g => { const L = lastL;
      ...
      if (Math.abs(g[0] - S.HOME[0] - 1) < 1.8 && Math.abs(g[1] - S.HOME[1] - 1) < 1.8) { const w = S.houseWord(state), n = S.nextHouseWord(state); bsay(S.landName(state) + ' · ' + E.eraOf(state).name, E.nextEra(state) ? (E.nextEra(state).n - E.score(state)) + ' more and it is ' + E.nextEra(state).word + '.' : 'A kingdom.'); Land.goTo(me, L.paths, [S.HOME[0] + 1.5, S.HOME[1] + 2.5]); me.follow = true; return; }
    ...
    const landRow = el('div', 'grp'); landRow.appendChild(el('span', null, 'Land')); Object.keys(E.BIOMES).forEach(k => { const b = el('button', null, E.BIOMES[k].name); b.type = 'button'; b.dataset.v = k; b.setAttribute('aria-pressed', String(state.biome === k)); b.addEventListener('click', () => { state.biome = k; save(); landRow.querySelectorAll('button').forEach(x => x.setAttribute('aria-pressed', String(x.dataset.v === k))); }); landRow.appendChild(b); }); opts.appendChild(landRow);
    ...
    views.avatar.appendChild(el('div', 'cap', 'The pineapple is the house avatar. Whatever you build here walks your land, on the roads. The land is scenery only: Hill Country, desert, forest or coast, it grows the same way.'));
    ...
    function paintBuild() {
      const era = E.eraOf(state), ne = E.nextEra(state), sc = E.score(state);
      tWb.textContent = era.name + ' · ' + sc + ' built'; tWs.textContent = ne ? (ne.n - sc) + ' more to ' + ne.word + ': ' + ne.needs : 'A kingdom, with everything'; tWi.style.width = (ne ? (sc - era.n) / (ne.n - era.n) * 100 : 100) + '%';
    ...
    function fit() { const cw = Math.max(240, wrap.clientWidth - 24), ch = hostEl.clientHeight || 0; const chrome = bar.offsetHeight + (devbar.hidden ? 0 : devbar.offsetHeight); const avail = Math.max(300, ch - chrome - 40);
      SCENES.forEach(([I, sc, share]) => {
        ...
        const view = sc.sc.parentElement; let extras = 0; if (view && !view.hidden) Array.prototype.forEach.call(view.children, c => { if (c !== sc.sc && !c.hidden && !c.classList.contains('board') && !c.classList.contains('list') && c.tagName !== 'H3') extras += c.offsetHeight + 10; });
    ...
    /* ---- mode, dev controls ---- */
    function paintMode() { setTimeout(fit, 0); mode.textContent = state.mode === 'dev' ? 'Dev · simulate and reset' : 'Production · real data only'; mode.dataset.on = state.mode; devToggle.hidden = state.mode !== 'dev'; devbar.hidden = state.mode !== 'dev' || !devOpen; }
