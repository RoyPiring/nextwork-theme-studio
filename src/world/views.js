...
(function () {
  ...
  function mount(root, host) {
    ...
    function paintDev() { while (devbar.firstChild) devbar.removeChild(devbar.firstChild); devbar.appendChild(el('span', null, 'One at a time'));
      const nx = S.nextProject(state); const f = el('button', null, nx ? 'Build the next: ' + nx.title : 'Nothing left'); f.type = 'button'; f.disabled = !nx; f.addEventListener('click', () => nx && finish(nx.title, nx.series)); devbar.appendChild(f);
      const u = el('button', null, 'Undo the last'); u.type = 'button'; u.disabled = !state.done.length; u.addEventListener('click', () => { const t = state.done.pop(); delete state.sites[t]; save(); lastL = S.layout(state); paintBuild(); paintBoard(); paintDev(); bsay('Undone', t + ' is back on the path.'); }); devbar.appendChild(u);
    ...
    function frame(now) {
      ...
      if (current === 'world') { world.ctx.setTransform(1, 0, 0, 1, 0, 0); Land.sky({ ctx: world.ctx, W: 880, H: 620 }, night); world.reset(); world.night = night; HQ.drawHQ(world, state, now, {}); }
