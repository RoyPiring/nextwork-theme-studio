...
(function () {
  ...
  function mount(root, host) {
    ...
    drag(b1.cv, base, g => { const L = lastL;
      ...
      if (Math.abs(g[0] - S.HOME[0]) < 1.6 && Math.abs(g[1] - S.HOME[1]) < 1.6) { const w = S.houseWord(state), n = S.nextHouseWord(state); bsay(S.landName(state) + ' · ' + E.eraOf(state).name, E.nextEra(state) ? (E.nextEra(state).n - E.score(state)) + ' more and it is ' + E.nextEra(state).word + '.' : 'A kingdom.'); Land.goTo(me, L.paths, [S.HOME[0] + 0.5, S.HOME[1] + 2.5]); me.follow = true; return; }
