...
(function () {
  ...
  const metroCivic = [C('cityhall', 28, 39), C('station', 30, 20), { kind: 'rail', from: [0, 19], to: [40, 19] }, C('hospital', 36, 34), C('library', 36, 36), C('firestation', 38, 36), C('datacentre', 36, 39), C('school', 36, 29), C('store', 39, 29), C('community', 25, 36), C('park', 22, 39), C('stadium', 36, 45), C('university', 15, 23), C('solar', 8, 57), C('workshop', 12, 57), C('watertower', 13, 57), C('windmill', 13, 32), C('barn', 8, 45), C('coop', 11, 45), { kind: 'paddock', rect: [8, 46, 6, 3] }];
  const capitalCivic = [C('capitol', 28, 52), C('cityhall', 35, 51), C('library', 26, 51), C('park', 22, 51), C('station', 30, 20), { kind: 'rail', from: [0, 19], to: [40, 19] }, C('hospital', 36, 34), C('firestation', 38, 36), C('datacentre', 36, 39), C('school', 36, 29), C('store', 39, 29), C('community', 25, 36), C('park', 22, 39), C('stadium', 36, 45), C('university', 15, 23), C('solar', 8, 57), C('workshop', 12, 57), C('watertower', 13, 57), C('windmill', 13, 32), C('barn', 8, 45), C('coop', 11, 45), { kind: 'paddock', rect: [8, 46, 6, 3] }];
  ...
  const PLANS = [
    { id: 'campground', name: 'Campground', word: 'a campground', threshold: 0,
      ...
      lots: [row(32, 33, 39, 'south'), row(34, 34, 37, 'north'), row(32, 27, 27, 'south')],
    ...
    { id: 'fort', name: 'Fort', word: 'a fort', threshold: 5,
      ...
      civic: [{ kind: 'stockade', rect: [22, 27, 18, 13] }, C('campfire', 30, 34), C('pump', 32, 34), C('windmill', 25, 28), C('tank', 28, 28), C('workshop', 34, 28)],
    ...
    { id: 'town', name: 'Town', word: 'a town', threshold: 15,
      ...
      civic: [C('pump', 29, 34), C('townhall', 30, 39), C('school', 35, 34), C('store', 38, 34), C('community', 35, 37), C('windmill', 13, 32), C('tank', 42, 32), C('workshop', 42, 34), C('barn', 21, 39), C('coop', 24, 39), { kind: 'paddock', rect: [21, 40, 7, 3] }],
    ...
    { id: 'city', name: 'City', word: 'a city', threshold: 35,
      ...
      civic: [C('cityhall', 28, 39), C('hospital', 35, 34), C('library', 35, 36), C('firestation', 38, 36), C('school', 35, 29), C('store', 38, 29), C('community', 26, 36), C('datacentre', 42, 34), C('tank', 42, 32), C('workshop', 42, 30), C('windmill', 13, 32), C('park', 22, 39), C('barn', 8, 45), C('coop', 11, 45), { kind: 'paddock', rect: [8, 46, 6, 3] }],
    ...
    { id: 'metropolis', name: 'Metropolis', word: 'a metropolis', threshold: 60,
      ...
      roads: metroRoads(56), zones: [{ kind: 'plaza', rect: [28, 34, 6, 4] }], civic: metroCivic, lots: metroLots,
      landscape: { fields: FARMS, treeline: 'thin', lamps: 'all' } },
    { id: 'capital', name: 'Capital', word: 'the capital', threshold: 90,
      story: 'The plaza becomes the capitol lawn and a mall runs south on the axis of the home to the capitol, standing in its own margin between the capitol street and the south street. The cross streets stop at the mall; the old city hall is rebuilt on the capitol’s east flank with a library and park on the west; the grid takes one more street south; the whole west bank is lit.',
      home: [30, 31], clearing: { cx: 30, cy: 34, rx: 30, ry: 30 }, water: water(2, [{ y: 33, x0: 45, x1: 46 }], true, { x0: 43, x1: 44, y0: 20, y1: 62 }),
      roads: capitalRoads(), zones: [{ kind: 'park', rect: [28, 34, 6, 16] }], civic: capitalCivic, lots: capitalLots,
      landscape: { fields: FARMS, treeline: 'thin', lamps: 'all' } },
    { id: 'kingdom', name: 'Kingdom', word: 'a kingdom', threshold: 120,
      story: 'The capital holds on the west bank; the river is crossed a second time at the capitol street; the creek opens into a lake in the north; and a second city rises on the east bank round a royal square, the palace facing the main-street bridge with its own hall beside the square, both tied through to the east trunk.',
      ...
      civic: [C('palace', 50, 30), C('townhall2', 50, 34)].concat(capitalCivic), lots: capitalLots,
      landscape: { fields: FARMS, treeline: 'thin', lamps: 'all' } }
  ];
