...
(function () {
  ...
  const X = (W, x) => 34 + x / LANE * (W - 70), Y = (H, y) => H * 0.7 + (y - 0.5) * H * 0.2, SC = y => (0.85 + y * 0.3) * K;
  ...
  function drawUs(ctx, u, px, py, s, t) {
    ...
    if (u.id === 'wyrm') { const bone = u.flash ? '#fff' : '#e6e3d6', fl = py - 34 * s + Math.sin(t * 1.5) * 4 * s, flap = Math.sin(t * 4) * 12 * s; ell(ctx, px, py + 1, 22 * s, 5 * s, 'rgba(0,0,0,.25)');
      [[-1, 'rgba(70,50,110,.55)'], [1, 'rgba(90,70,140,.5)']].forEach(([side, c]) => { const bx = px - 2 * s, by = fl - 4 * s, tipx = px - 18 * s, tipy = fl - 34 * s - flap * side * 0.5; ctx.fillStyle = c; ctx.beginPath(); ctx.moveTo(bx, by); ctx.lineTo(tipx, tipy); ctx.lineTo(px - 30 * s, fl - 8 * s); ctx.lineTo(px - 14 * s, fl + 2 * s); ctx.closePath(); ctx.fill(); ln(ctx, [bx, by], [tipx, tipy], bone, 2.2 * s); [0.35, 0.65].forEach(k => ln(ctx, [tipx + (bx - tipx) * k, tipy + (by - tipy) * k], [px - 30 * s + (px - 14 * s - (px - 30 * s)) * k, fl - 8 * s + 10 * s * k], bone, 1.2 * s)); });
      ...
      ctx.fillStyle = bone; ctx.beginPath(); ctx.moveTo(px + 8 * s, fl - 14 * s); ctx.lineTo(px + 26 * s, fl - 13 * s); ctx.lineTo(px + 30 * s, fl - 8 * s); ctx.lineTo(px + 16 * s, fl - 5 * s); ctx.closePath(); ctx.fill(); ctx.beginPath(); ctx.moveTo(px + 10 * s, fl - 14 * s); ctx.lineTo(px + 6 * s, fl - 24 * s); ctx.lineTo(px + 14 * s, fl - 15 * s); ctx.closePath(); ctx.fill();
      ln(ctx, [px + 16 * s, fl - 5 * s], [px + 28 * s, fl - 3 * s + Math.sin(t * 5) * 2 * s], bone, 2 * s); blob(ctx, px + 18 * s, fl - 11 * s, 2 * s, '#1c1a24'); blob(ctx, px + 18.5 * s, fl - 11 * s, 1.3 * s, '#7ff5ff'); const eg = ctx.createRadialGradient(px + 18 * s, fl - 11 * s, 0, px + 18 * s, fl - 11 * s, 8 * s); eg.addColorStop(0, 'rgba(127,245,255,.5)'); eg.addColorStop(1, 'rgba(127,245,255,0)'); ctx.fillStyle = eg; ctx.beginPath(); ctx.arc(px + 18 * s, fl - 11 * s, 8 * s, 0, Math.PI * 2); ctx.fill(); return; }
  ...
  function draw(ctx, W, H, b, now, look) {
    ...
    if (b && b.domainT > 0) { const a = Math.min(1, b.domainT, 6 - b.domainT + 0.2); const dg = ctx.createLinearGradient(0, H * 0.5, 0, H); dg.addColorStop(0, 'rgba(90,20,140,0)'); dg.addColorStop(0.4, 'rgba(90,20,140,' + 0.45 * a + ')'); dg.addColorStop(1, 'rgba(40,0,70,' + 0.7 * a + ')'); ctx.fillStyle = dg; ctx.fillRect(0, H * 0.5, W, H * 0.5); for (let i = 0; i < 26; i++) { const x = (i * 97 + t * 30 * (i % 3 + 1)) % W, y = H - ((t * 40 + i * 37) % (H * 0.45)); blob(ctx, x, y, 2 + (i % 3), 'rgba(200,140,255,' + 0.6 * a + ')'); } }
